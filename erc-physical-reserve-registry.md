...
A physical reserve may represent a gold bar, precious-metal batch, warehouse receipt, custody record, or another real-world reserve object. The standard defines a common observable model for reserve identity, asset type, quantity, state, endorsements, and optional reserve metadata.

This ERC does not prescribe how reserves are originated, deposited, audited, legally held, or operationally managed. Implementations may use custodians, banks, regulators, auditors, or oracle feeds. The purpose of this ERC is to expose the minimum reserve accounting surface that tokens, wallets, auditors, explorers, and protocols require.
...
Reserve-backed real-world asset systems often rely on reserve records that are operationally important but not exposed through a common on-chain interface. A project may maintain bank confirmations, vault records, warehouse receipts, audit reports, or regulatory attestations, but external systems usually cannot query a standardized contract interface to determine:

- what physical reserves exist;
- which asset type those reserves represent;
- how much reserve quantity is active;
- how much reserve has been allocated to a token contract or other instrument;
- how much reserve remains available for future allocation;
- how much reserve has been consumed or otherwise removed from future backing;
- which authorities, custodians, auditors, or other parties have endorsed a reserve.

Existing RWA-related standards address adjacent concerns such as permissioned transfers, identity, compliance checks, or NFT redemption metadata. They do not define a reusable physical reserve-accounting layer that can be shared by reserve-backed tokens, auditors, wallets, explorers, and protocols.
...
The standard is not intended to replace bank records, custodian systems, audit procedures, vault operations, or legal documentation. Instead, it provides a regulation-agnostic, machine-readable on-chain interface that can mirror or reference those processes.
...
This ERC primarily uses `MUST`, `SHOULD`, and `MAY` for normative requirements. `SHALL` is equivalent to `MUST`, `RECOMMENDED` is equivalent to `SHOULD`, and `OPTIONAL` is equivalent to `MAY`.
...
This ERC is designed to be compatible with real custody and inventory practices. Where applicable, implementers may align metadata and documentation with existing precious-metal or commodity custody practices, such as bar lists, vault records, assay reports, custody statements, and other recognized inventory documents. This ERC does not normatively require any specific external custody standard.
...
This ERC does not require a separate reallocation function.

A reserve quantity MAY be reallocated from one reserve to another or from one instrument to another by composing existing allocation functions:

```text
releaseReserve(oldReserveId, instrument, quantity)
allocateReserve(newReserveId, instrument, quantity)
```

or:

```text
releaseReserve(reserveId, oldInstrument, quantity)
allocateReserve(reserveId, newInstrument, quantity)
```

Implementations SHOULD ensure that reallocation does not cause temporary over-allocation or under-backing for any dependent instrument.
...
Changing a reserve to `ACTIVE` signals that it may be used as backing. Implementations should restrict state transitions to authorized actors and should carefully validate all internal legal, operational, audit, or endorsement conditions before activation.
...
7. Releasing allocation from a `SUSPENDED` reserve does not necessarily make it available for new allocation.
8. Consumption is only possible from the allocated quantity.
9. Consumption decreases the allocated quantity and increases the consumed quantity.
10. Full consumption changes the reserve state to `CONSUMED`.
11. `CONSUMED` and `CANCELLED` reserves are not counted in active quantity.
12. `SUSPENDED` reserves are not available for new allocation.
13. Multiple authorities can endorse the same reserve under different endorsement types.
14. The same authority cannot be counted twice for the same reserve and endorsement type.
15. Optional metadata references return stable document hashes.
16. Optional ERC-721 receipt links correctly map `reserveId` to `(receiptContract, tokenId)` and back.
17. Fractional consumption of a reserve does not require burning the reserve receipt NFT.
18. Whole-reserve consumption may burn, transfer, lock, or mark the reserve receipt NFT according to the implementation policy.
